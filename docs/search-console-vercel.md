...
`genjustuai.vercel.app` é subdomínio da Vercel. Não dá para verificar o domínio `vercel.app`. Só o **prefixo de URL**.
...
2. Adicionar propriedade → **Prefixo do URL** → `https://genjustuai.vercel.app/` (barra no fim).
...
4. Na Vercel → Project → Settings → Environment Variables:
   ...
   - Production + Preview
5. **Redeploy** o `main`.
6. View Source em https://genjustuai.vercel.app/ e confirme a meta `google-site-verification`.
7. Na Search Console, **Verificar**.
8. No OAuth, homepage exatamente `https://genjustuai.vercel.app` ou `https://genjustuai.vercel.app/` — o mesmo da propriedade.
9. Authorized domain: `genjustuai.vercel.app` (não `vercel.app`).
